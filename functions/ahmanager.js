        ...
        if (isauctionactive === true && auctionendtime < currenttime) {
            ...
            for (let j = 0; j < notifusers.length; j++) {
                ...
                if (msguser === auctopbidder) {
                    ...
                    if (auctopbidder === 0) client.users.send(msguser, ({ content: redtext(`Your auction #${auctionid} has ended but with no bids!`) })).catch((e) => client.channels.fetch(botchannelid).then(channel => channel.send(`> <@${msguser}> Your auction #${auctionid} has ended but with no bids!\nPlease enable DMs with the bot!`)))
            ...
            for (let j = 0; j < notifusers.length; j++) {
                ...
                client.users.send(msguser, { content: bluetext(`The auction #${auctionid} is going to end in an hour!`) + `\n` + genmessagelink(id), embeds: [module.exports.embedgen(auctionid)], files: [attachment] }).catch((e) => client.channels.fetch(botchannelid).then(channel => channel.send(`<@${msguser}> I tried to message you in DMs, but I couldn't! Please unblock or enable DMs!`)))
            }
        }
